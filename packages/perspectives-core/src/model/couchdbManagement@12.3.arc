...
domain model://perspectives.domains#CouchdbManagement@12.3
  ...
  case CouchdbServer
    ...
    -- It requires write access to cw_servers_and_repositories.
    user Accounts (unlinked, relational) filledBy (sys:TheWorld$PerspectivesUsers + sys:SocialEnvironment$Persons)
      -- WithCredentials$Password
      -- WithCredentials$SpecificUserName
      -- WithCredentials$AuthorizedDomain
  ...
  case ModelManifest 
    ...
    user Author (relational) filledBy (Repository$Authors, Repository$Admin)
      perspective on extern
        props (Description, IsLibrary, VersionToInstall, RecomputeVersionToInstall, ModelManifest$External$IsSystemModel) verbs (Consult, SetPropertyValue)
    ...
    public Visitor at extern >> PublicUrl = sys:Me
      perspective on extern
        props (DomeinFileName, Description, IsLibrary, ModelManifest$External$IsSystemModel, VersionToInstall) verbs (Consult)
      -- NOTA BENE: dit betekent dat installaties proberen instanties van ModelsInUse te publiceren!
      perspective on sys:MySystem >> ModelsInUse
        only (Fill, Remove)
        props (ModelToRemove) verbs (SetPropertyValue)
  ...
  case VersionedModelManifest
    ...
    -- A versioned model can have any number of conversation source documents.
    -- Each role carries one modeller-authored YAML file; the build action below
    -- compiles the complete collection into one conversations.json attachment.
