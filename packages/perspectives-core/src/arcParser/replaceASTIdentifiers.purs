...
instance ReplaceIdentifiers RoleE where
  replaceIdentifier original addendum (RoleE r@({ id, roleParts })) = RoleE
    ( r
...
instance ReplaceIdentifiers StateSpecification where
  replaceIdentifier original addendum (SubjectState rid mp) = SubjectState
    ...
    ( case mp of
        Nothing -> Nothing
        Just segments ->
          if endsWithSegments segments original then Just $ segments <> addendum
          else Just segments
    )
...
instance ReplaceIdentifiers RoleType where
  replaceIdentifier original addendum r@(ENR (EnumeratedRoleType id)) =
    if endsWithSegments id original then ENR $ EnumeratedRoleType (id <> addendum)
    else r
  replaceIdentifier original addendum r@(CR (CalculatedRoleType id)) =
    if endsWithSegments id original then ENR $ EnumeratedRoleType (id <> addendum)
    else r
