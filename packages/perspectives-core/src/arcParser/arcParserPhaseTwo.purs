...
traverseContextE (ContextE { id, kindOfContext, public, contextParts, pos }) ns = do
  ...
  modelName contextId =
    if ns == "domain" && test newModelRegex contextId then contextId
  ...
  handleParts (Context contextUnderConstruction@{ contextAspects }) (ContextAspect contextName pos') = do
    ...
    if isTypeUri expandedAspect then pure (Context $ contextUnderConstruction { contextAspects = cons (ContextType expandedAspect) contextAspects })
  ...
  insertRoleInto (E (EnumeratedRole { id: roleId, kindOfRole })) c = case kindOfRole, c of
    ...
    TI.UserRole, (Context cr@{ gebruikerRol }) -> Context $ cr
      { gebruikerRol = case elemIndex (ENR roleId) gebruikerRol of
          Nothing -> cons (ENR roleId) gebruikerRol
          (Just _) -> gebruikerRol
      }
    -- This is the Enumerated variant of the public role:
    TI.PublicProxy, (Context cr@{ gebruikerRol }) -> Context $ cr
      { gebruikerRol = case elemIndex (ENR roleId) gebruikerRol of
          Nothing -> cons (ENR roleId) gebruikerRol
          (Just _) -> gebruikerRol
      }
...
traverseRoleE r ns =
  ...
  isCalculatedRole (RoleE { roleParts }) =
    ( isJust
        ( findIndex
            ( case _ of
...
traverseEnumeratedRoleE_ role@(EnumeratedRole { id: rn, kindOfRole }) roleParts = do
  ...
  -- FILLEDBYATTRIBUTE
  handleParts roleName (EnumeratedRole roleUnderConstruction@{ id, binding }) (FilledBySpecifications (spec :: TypeCombination)) = do
    spec' <-
      if kindOfRole == TI.ContextRole then
        case spec of
          Alternatives attrs -> pure $ Alternatives (f <$> attrs)
          Combination attrs -> pure $ Combination (f <$> attrs)
          DisjunctionOfConjunctions groups -> pure $ DisjunctionOfConjunctions $ (\group -> f <$> group) <$> groups
      else pure spec
    bindingRestriction <- compileRoleTypeCombination id spec'
    pure (EnumeratedRole $ roleUnderConstruction { binding = Just bindingRestriction })

    where
    f (FilledByAttribute bnd context) = FilledByAttribute (externalRoleType_ bnd) context
  ...
  -- SCREENE
  handleParts roleName e@(EnumeratedRole roleUnderConstruction@{ kindOfRole: kind }) (Screen s) =
    if kind == TI.UserRole || kind == TI.PublicProxy then do
...
traverseCalculatedRoleE_ role@(CalculatedRole { id: roleName, kindOfRole }) roleParts = do
  ...
  -- SCREENE
  handleParts e@(CalculatedRole roleUnderConstruction@{ kindOfRole: kind }) (Screen s) =
    if kind == TI.UserRole || kind == TI.Public then do
      screen' <- expandPrefix s
      void $ lift $ modify \st@{ screens } -> st { screens = Cons screen' screens }
      pure e
    else case s of
...
traversePropertyE r ns =
  ...
  isCalculatedProperty (PropertyE { propertyParts }) =
    ( isJust
        ( findIndex
            ( case _ of
                (Calculation' _ _) -> true
