...
compileStatement originDomain currentcontextDomain userRoleTypes statements =
  ...
  describeAssignmentStatement subjects ass = case ass of
    RemoveRole { roleExpression, start, end } -> do
      ...
      for_ subjects
        ( \subject -> for_ (roleInContext2Role <$> (allLeavesInADT $ unsafePartial roleRange rle))
    ...
    CreateRole { roleIdentifier, contextExpression, localName, start, end } -> do
      ...
      for_ subjects
        ( \subject -> (lift $ lift $ roleHasPerspectiveOnRoleWithVerb subject qualifiedRoleIdentifier [ Verbs.Create ] (Just start) (Just end)) >>= case _ of
            Left e -> throwError e
            _ -> pure unit
        )
        ...
        Just r -> do
          ...
          case qualifiedRoleIdentifier of
            CR calculatedType -> do
              ...
              if isDBQRole then case mnameGetterDescription of
                Nothing -> pure $ UQD
                  originDomain
            ...
            ENR enumeratedType -> do
              ...
              for_ subjects
                ( \subject -> (lift $ lift $ roleHasPerspectiveOnRoleWithVerb subject enumeratedType [ Verbs.CreateAndFill ] (Just start) (Just end)) >>= case _ of
                    Left e -> throwError e
                    _ -> pure unit
                )
              case mnameGetterDescription of
    ...
    Bind f@{ bindingExpression, roleIdentifier, contextExpression, start, end } -> do
      ...
      for_ subjects
        ( \subject -> (lift $ lift $ roleHasPerspectiveOnRoleWithVerb subject qualifiedRoleIdentifier [ Verbs.Fill, Verbs.CreateAndFill ] (Just start) (Just end)) >>= case _ of
            Left e -> throwError e
            _ -> pure unit
        )
    ...
    DeleteRole f@{ roleIdentifier, contextExpression, start, end } -> do
      ...
      for_ subjects
        ( \subject -> (lift $ lift $ roleHasPerspectiveOnRoleWithVerb subject qualifiedRoleIdentifier [ Verbs.Delete ] (Just start) (Just end)) >>= case _ of
            Left e -> throwError e
            _ -> pure unit
        )
    ...
    DeleteContext f@{ contextRoleIdentifier, contextExpression, start, end } -> do
      ...
      case qualifiedRoleType of
        ENR qualifiedRoleIdentifier ->
          -- Check for each of the subjects whether they have a sufficient perspective to Delete the qualifiedRoleIdentifier.
          for_ subjects
            ( \subject -> (lift $ lift $ roleHasPerspectiveOnRoleWithVerb subject qualifiedRoleIdentifier [ Verbs.DeleteContext ] (Just start) (Just end)) >>= case _ of
                Left e -> throwError e
                _ -> pure unit
            )
        _ -> pure unit
      kindOfRole <- (lift $ lift $ roleKindOfRoleType qualifiedRoleType)
      if kindOfRole == ContextRole then pure $ UQD originDomain (QF.DeleteContext qualifiedRoleType) cte originDomain True True
      else throwError $ NotAContextRole start end
    ...
    ExternalEffect f@{ start, end, effectName, arguments } -> case lookupHiddenFunction effectName of
      ...
      Just _ ->
        if lookupHiddenFunctionIsEffect effectName then do
    ...
    qualifyContextType contextIdentifier start end =
      if isTypeUri contextIdentifier then pure $ ContextType contextIdentifier
      else do
