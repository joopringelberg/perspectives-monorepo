...
getPropertyBearingRoleInstance prop roleId = do
  ...
  case lookup (unwrap prop) aliases of
    ...
    Nothing -> do
      ...
      if isJust $ elemIndex (ENP prop) allProps then pure $ Just $ RoleProp roleId prop
...
roleContextualisations ctxt qualifiedRoleIdentifier = do
  ...
  roleTypesToCreate <-
    if (unwrap qualifiedRoleIdentifier) `startsWithSegments` (unwrap cType) then pure [ qualifiedRoleIdentifier ]
  ...
  roleTypesToCreate' <-
    if null roleTypesToCreate then pure [ qualifiedRoleIdentifier ]
...
isUntouchable ctxts rls (RoleAssumption c r) = ST.evalStateT ((for_ rls f) *> ST.get) false
  ...
  f rl = do
    ...
    if not isUntouchable_ then do
      rle <- lift $ getPerspectRol rl
      if c == rol_context rle && r == rol_pspType rle then ST.put true
      else pure unit
    else pure unit
