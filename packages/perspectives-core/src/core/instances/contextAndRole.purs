...
deleteContext_rolInContext (PerspectContext ct@{ rolInContext }) (EnumeratedRoleType rolName) = PerspectContext
  ( ct
...
pushContext_state (PerspectContext cr@{ states }) stateId = PerspectContext cr
  { states =
...
defaultContextRecord =
  ...
  , universeContextDelta: SignedDelta { author: PerspectivesUser "", encryptedDelta: "UniverseContextDelta from defaultContextRecord", signature: Nothing, sequenceNumber: Nothing }
  , states: []
  , publicUrl: Nothing
  }
...
defaultRolRecord =
  ...
  , occurrence: 0
  , isMe: false
  , universeRoleDelta: SignedDelta { author: PerspectivesUser "", encryptedDelta: "UniverseRoleDelta from defaultRolRecord", signature: Nothing, sequenceNumber: Nothing }
  , contextDelta: SignedDelta { author: PerspectivesUser "", encryptedDelta: "ContextDelta from defaultRolRecord", signature: Nothing, sequenceNumber: Nothing }
    ...
    ( \cr ->
        ...
        in
          case lookup cIndex cr.filledRoles of
            Nothing -> cr
              { filledRoles = insert cIndex (singleton rIndex [ filled ]) cr.filledRoles
              , roleAliases = roleAliases'
              , contextAliases = contextAliases'
              }
            Just (roleMap :: Object (Array RoleInstance)) -> case lookup rIndex roleMap of
              Nothing -> cr
                { filledRoles = insert cIndex (insert rIndex [ filled ] roleMap) cr.filledRoles
                , roleAliases = roleAliases'
                , contextAliases = contextAliases'
                }
              (Just roles) -> do
                case Arr.elemIndex filled roles of
                  Nothing -> cr
                    { filledRoles = insert cIndex (insert rIndex (Arr.union [ filled ] roles) roleMap) cr.filledRoles
                    , roleAliases = roleAliases'
                    , contextAliases = contextAliases'
                    }
                  _ -> cr
...
removeRol_gevuldeRollen ct cType rolName rolID =
  let
        ...
        Just roleMap -> case lookup rIndex roleMap of
          ...
          (Just (roles :: Array RoleInstance)) -> do
            case Arr.elemIndex rolID roles of
              ...
              _ ->
                let
                  roles' = Arr.delete rolID roles
                in
                  if null roles' then cr { filledRoles = insert cIndex (delete rIndex roleMap) cr.filledRoles }
...
pushRol_state (PerspectRol cr@{ states }) stateId = PerspectRol cr
  { states =
      case Arr.elemIndex stateId states of
        Nothing -> Arr.snoc cr.states stateId
        Just _ -> states
...
getNextRolIndex rolIds =
  case
    ( maximum $ rolIds <#> \(RoleInstance id) -> case lastIndexOf (Pattern "_") id of
        Nothing -> 0
        Just n ->
          let
            { after } = splitAt (n + 1) id
          in
