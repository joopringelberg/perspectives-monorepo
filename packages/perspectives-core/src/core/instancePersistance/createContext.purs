...
import Perspectives.Logging (traceResource)
...
constructEmptyContext contextInstanceId ctype localName externeProperties authorizedRole = do
  lift $ lift (humanizePerspectivesWarning (ConstructContext (ContextType ctype) authorizedRole contextInstanceId) >>= traceResource <<< show)
