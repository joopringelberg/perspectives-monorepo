...
scheduleRoleRemoval sync id = do
  ...
  f = do
    ...
    contextIsScheduledToBeRemoved <- gets
      ( \(Transaction { scheduledAssignments }) -> isJust $ find
          ( case _ of
    ...
    if contextIsScheduledToBeRemoved || roleIsUntouchable then pure []
    else do
      modify
        ( over Transaction \t@{ scheduledAssignments, rolesToExit } -> t
      ...
      if sync then (lift $ try $ (getPerspectRol id)) >>= handlePerspectRolError' "scheduleRoleRemoval" []
...
scheduleContextRemoval authorizedRole usersWithPerspectiveOnEmbeddingRole id =
  ...
  -- In that case, we remove the instance for the first public role (in a random order). 
  -- We should check whether it is still there before we try to remove it.
  if isInPublicScheme (unwrap id) then (lift $ tryGetPerspectContext id) >>= case _ of
    Nothing -> pure unit
    Just _ -> f
  else f
  where
    ...
    handlePerspectContextError "scheduleContextRemoval"
      \(ctxt@(PerspectContext { rolInContext, buitenRol: br, pspType: contextType })) -> do
        ...
        signedDelta <- signDelta $ writeJSON $ stripResourceSchemes $ UniverseRoleDelta
          ...
          , authorizedRole
          -- Note that in this case, the authorizedRole is NOT the type of the roleInstance.
          , roleInstance: context_buitenRol ctxt
        ...
        for_ allItems \item -> do
          ...
          case mroleOnClipboard of
            Nothing -> pure unit
            Just roleOnClipboard ->
              if isJust $ elemIndex roleOnClipboard allRoleInstances then void $ scheduleRoleRemoval false {-do not share with peers-}  item
              else pure unit
        modify
          ( over Transaction \t@{ scheduledAssignments, rolesToExit } -> t
...
queryUpdatesForRoleRemoval role@(PerspectRol { id: roleId, pspType: roleType, context: contextId }) = do
  ...
  if rol_isMe role then (lift $ findMeRequests contextId)
...
severeBindingLinks (PerspectRol { id: roleId, pspType: roleType, binding, filledRoles }) = do
  ...
  case binding of
    ...
    Just b ->
      if isInPublicScheme (unwrap b) && not isInPublicScheme (unwrap roleId) then pure unit
  ...
  handlePerspectRolError' "changeRoleBinding" unit
    \(filled :: PerspectRol) -> do
      ...
      case rol_binding filled of
        Just filler ->
          if isInPublicScheme (unwrap filler) && not isInPublicScheme (unwrap filledId) then pure unit
      ...
      else if filledIsMe then case mNewFiller of
        Nothing -> do
