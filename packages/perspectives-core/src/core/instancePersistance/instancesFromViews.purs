...
getSafeViewOnDatabase :: forall a i key. 
  Attachment a => 
  Persistent a i => 
  ReadForeign key =>
  WriteForeign key =>
  ReadForeign i => 
  DatabaseName -> ViewName -> Keys key -> MonadPerspectives (Array i)
getSafeViewOnDatabase = getSafeViewOnDatabase_ identity

getSafeViewOnDatabase_ :: forall a i key result. 
  Attachment a => 
  Persistent a i => 
  ReadForeign key =>
  WriteForeign key =>
  ...
  ReadForeign result =>
  (result -> i) ->
  DatabaseName -> ViewName -> Keys key -> MonadPerspectives (Array result)
