...
getNodes = do
  ...
  case res of
    ...
    Right (response :: Response String) ->
      if response.status == StatusCode 200 then case (readJSON response.body) of
...
createUser user userName = do
  ...
  case res of
    ...
    Right (response :: Response String) ->
      if response.status == StatusCode 201 then pure unit
  ...
  case head nodes of
    ...
    Just node -> do
      ...
      case res of
        ...
        Right (response :: Response String) ->
          if response.status == StatusCode 201 || response.status == StatusCode 204 then pure unit
...
selfRegisterWithRabbitMQ_ brokerServiceUrl userName password queueName = do
  ...
  case res of
    ...
    Right (response :: Response Json) ->
      if response.status == StatusCode 200 || response.status == StatusCode 201 then pure unit
