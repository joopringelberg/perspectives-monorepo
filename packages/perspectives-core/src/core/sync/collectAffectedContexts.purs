...
usersWithPerspectiveOnRoleInstance roleType roleInstance contextInstance runForwards = do
  ...
  users2 <- do
    ...
    (roleCalculations :: (Array InvertedQuery)) <- lift
      ( unsafePartial runTimeIndexForRoleQueries roleType contextInstance
...
handleBackwardQuery roleInstance iq@(InvertedQuery { description, backwardsCompiled, users: userTypes, states, forwardsCompiled, perspectiveStartPosition }) = catchError
  ( case backwardsCompiled of
      ...
      _ -> do
        ...
        else if unsafePartial shouldResultInRoleStateQuery iq then createRoleStateQuery *> pure []
...
usersWithAnActivePerspective roleInstance iq@(InvertedQuery { description, backwardsCompiled, users: userTypes, states, forwardsCompiled }) =
  if unsafePartial $ backwardsQueryResultsInRole iq then fromRoleResults
  else if unsafePartial $ backwardsQueryResultsInContext iq then fromContextResults
  else throwError
    ( error
        "Programming error in handleBackwardQuery.usersWithAnActivePerspective: query should result\
      ...
      case _ of
        ...
        Just (State.State { stateFulObject }) ->
          case stateFulObject of
            ...
            State.Cnt _ -> (lift $ contextIsInState stateId cid) >>=
              if _ then lift $ flip cons accumulatedUsers <<< Tuple cid <$> (join <$> traverse (\userType -> (cid ##= getRoleInstances userType) >>= filterA notIsMe) userTypes)
            ...
            State.Orole rtype -> do
              ...
              lift (findM (roleIsInState stateId) objects) >>= \mObject ->
                if isJust mObject then lift $ flip cons accumulatedUsers <<< Tuple cid <$> (join <$> traverse (\userType -> (cid ##= getRoleInstances userType) >>= filterA notIsMe) userTypes)
                else pure accumulatedUsers
      ...
      case _ of
        ...
        Just (State.State { stateFulObject }) ->
          case stateFulObject of
            ...
            State.Srole _ -> (lift $ roleIsInState stateId rid) >>=
              if _ then lift (rid ##>> OG.context) >>= (\cid -> pure $ cons (Tuple cid [ rid ]) accumulatedUsers)
              else pure accumulatedUsers
            ...
            State.Orole _ -> (lift $ roleIsInState stateId rid) >>=
              if _ then do
                cid <- lift (rid ##>> OG.context)
                flip cons accumulatedUsers <<< Tuple cid <$> lift (join <$> traverse (\userType -> (cid ##= getRoleInstances userType) >>= filterA notIsMe) userTypes)
              else pure accumulatedUsers
...
runForwardsComputation roleInstance (InvertedQuery { description, forwardsCompiled, statesPerProperty, states }) cwus =
  if null (filter (\(Tuple context users) -> not $ null users) cwus) then pure []
  else do
    case forwards description of
      ...
      Just fw -> do
        ...
        if isRoleDomain $ range fw then computeProperties rinstances statesPerProperty cwus
        else pure unit
    pure $ join $ snd <$> cwus
...
createDeltasFromAssumption users (FilledRolesAssumption fillerId filledContextType filledType) = do
  ...
  case head filledRoles of
    Nothing -> pure unit
    Just someFilled -> (lift $ tryGetPerspectEntiteit someFilled) >>=
      ( case _ of
...
aisInPropertyDelta
  ...
  = (lift $ propertyTypeIsAuthorOnly $ ENP replacementProperty) >>=
  if _ then pure []
  else do
...
addDeltasForPropertyChange roleWithPropertyValue property replacementProperty = do
  ...
  nub <<< concat <$> for allFilleds \roleWithPropertyValue' -> do
    ...
    nub <<< concat <$> for contextCalculations \iq@(InvertedQuery { statesPerProperty }) -> do
      ...
      cwus' :: Array ContextWithUsers <- pure (filter (\(Tuple context users) -> not $ null users) cwus)
      if null cwus'
