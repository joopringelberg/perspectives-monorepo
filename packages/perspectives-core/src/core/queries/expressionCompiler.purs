...
makeRoleGetter currentDomain@(CDOM contextAdt) rt@(ENR et) = do
  ...
  isF <- lift2 $ roleTypeIsFunctional rt
  isM <- lift2 $ roleTypeIsMandatory rt
  if unlinked then pure $ SQD currentDomain (QF.DataTypeGetterWithParameter QF.GetRoleInstancesForContextFromDatabaseF (unwrap et)) (RDOM $ context2RoleInContextADT contextAdt et) (bool2threeValued isF) (bool2threeValued isM)
...
qualifyLocalRoleName pos ident = do
  ...
  if isTypeUri ident then case lookup ident enumeratedRoles of
...
qualifyLocalContextName_ pos ident roleIdentifiers = do
  ...
  case head candidates of
    ...
    (Just qname) | length candidates == 1 -> pure qname
    otherwise -> throwError $ NotUniquelyIdentifyingContext pos (ContextType ident) (ContextType <$> candidates)
...
compileAndSaveProperty dom step (CalculatedProperty cp@{ id, role, pos }) considerFunctional = withFrame do
  ...
  if loops then throwError $ (RecursiveDefinition $ loopErrorMessage (Prop id) pos pos)
  else withCurrentCalculation (Prop id)
    do
      -- We add the role as the variable "currentobject"
      kindOfRole <- unsafePartial $ roleKind role
      if kindOfRole == RTI.UserRole && stepContainsVariableReference "currentobject" step then throwError (CurrentObjectNotAllowed (startOf step) (endOf step))
      else pure unit
      expressionWithEnvironment <- pure $ addContextualBindingsToExpression
...
compileSimpleStep currentDomain s@(ArcIdentifier pos ident) = do
  -- Obviously, 'ident' will be a Readable identifier. It also must, by definition of the language, be fully qualified.
  -- Only after phase three will we replace it by a Stable identifier.
    ...
    RDOM (r :: ADT RoleInContext) -> do
      ...
      case madtOfBinding of
        Just adtOfBinding -> case membeddingContext of
          ...
          Just context ->
            if isTypeUri context then pure $ SQD currentDomain (QF.DataTypeGetterWithParameter FillerF context) (RDOM $ replaceContext adtOfBinding (ContextType context)) True False
  ...
  case currentDomain of
    (RDOM (adtOfBinder :: ADT RoleInContext)) -> do
      case membeddingContext of
        ...
        Just context ->
          if isTypeUri context then pure $ SQD currentDomain (QF.FilledF qBinderType (ContextType context)) (RDOM $ replaceContext adtOfBinder (ContextType context)) False False
          -- Try to qualify the name within the Domain.
          else do
            { id } <- lift $ gets _.dfr
            -- The namespace is Readable here, but we can safely pretend it is Stable, because we have the DomeinFile in cache in Readable form.
...
compileSimpleStep currentDomain s@(RoleTypeIndividual pos typeName) = do
  ...
  case length typeCandidates, head typeCandidates of
    ...
    _, _ -> throwError $ NotUniquelyIdentifyingRoleType pos (ENR $ EnumeratedRoleType typeName) typeCandidates
...
compileSimpleStep currentDomain s@(ContextTypeIndividual pos typeName) = do
  ...
  case length typeCandidates, head typeCandidates of
    ...
    _, _ -> throwError $ NotUniquelyIdentifyingContext pos (ContextType typeName) typeCandidates
...
compileSimpleStep currentDomain s@(SpecialisesRoleType pos roleName) = do
  case currentDomain of
    RoleKind -> do
      ...
      (qRoleName :: EnumeratedRoleType) <-
        if isTypeUri roleName then pure $ EnumeratedRoleType roleName
        -- Try to qualify the name within the Domain.
        else do
          { id } <- lift $ gets _.dfr
          -- The namespace is Readable here, but we can safely pretend it is Stable, because we have the DomeinFile in cache in Readable form.
...
compileSimpleStep currentDomain s@(IsInState pos stateName) = do
  ...
  f namespaces =
    if isTypeUri stateName then pure $ StateIdentifier stateName
    else do
      ...
      allMatchingStateNames <- pure $ foldMap
        ( \ns -> case lookup (qualifyWith ns stateName) states of
            Nothing -> []
            Just _ -> [ (qualifyWith ns stateName) ]
        )
        namespaces
      case length allMatchingStateNames of
        ...
        0 -> case (filter (\id -> id `endsWithSegments` stateName) (keys states)) of
          ...
          multiple -> throwError $ NotUniquelyIdentifyingState pos (StateIdentifier stateName) (StateIdentifier <$> keys states)
        1 -> pure $ StateIdentifier $ unsafePartial $ fromJust (head allMatchingStateNames)
        _ -> throwError $ NotUniquelyIdentifyingState pos (StateIdentifier stateName) (StateIdentifier <$> allMatchingStateNames)
...
compileSimpleStep currentDomain (Variable pos varName) = do
  ...
  case mBinding of
    ...
    (Just fdesc) -> do
      isF <-
        if isJust $ elemIndex varName [ "currentcontext", "origin", "currentactor", "notifieduser" ] then pure True
...
compileRoleTypeExpression stp = case stp of
  ...
  Binary (BinaryStep { operator, left, right }) -> case operator of
    ...
    _ -> throwError $ Custom "Type expression must use role type names combined with `union` or `intersection`."
  _ -> throwError $ Custom "Type expression must use role type names combined with `union` or `intersection`."
  where
...
compileContextTypeExpression stp = case stp of
  ...
  Binary (BinaryStep { operator, left, right }) -> case operator of
    ...
    _ -> throwError $ Custom "Type expression must use context type names combined with `union` or `intersection`."
  _ -> throwError $ Custom "Type expression must use context type names combined with `union` or `intersection`."
  where
...
compileBinaryStep currentDomain s@(BinaryStep { operator, left, right }) =
  case operator of
    ...
    Union pos -> do
      ...
      if equalDomainKinds (range f1) (range f2) then pure $ BQD currentDomain (QF.BinaryCombinator UnionF) f1 f2 (unsafePartial $ fromJust $ sumOfDomains (range f1) (range f2)) False (THREE.and (mandatory f1) (mandatory f2))
      else throwError $ IncompatibleDomains (startOf left) (endOf right)
    Intersection pos -> do
      f1 <- compileStep currentDomain left
      f2 <- compileStep currentDomain right
    ...
    BindsOp pos -> do
      f1 <- compileStep currentDomain left
      f2 <- compileStep currentDomain right
      if (pessimistic $ functional f1) then
        if (pessimistic $ functional f2) then case (range f1), (range f2) of
  ...
  ensureDomainIsRange d@(VDOM r p) allowedRangeConstructors pos fd =
    if (isJust $ elemIndex r allowedRangeConstructors) -- && (isJust p) Don't know why I originally added this constraint?
    then fd
  ...
  binOp pos left' right' allowedRangeConstructors functionName =
    if (pessimistic $ functional left') && (pessimistic $ functional right') then case range left', range right' of
      ...
      d1@(VDOM rc1 _), (VDOM rc2 _) | (isDate rc1 && isPDuration rc2 || isPDuration rc1 && isDate rc2) ->
        if isPMonth rc1 || isPMonth rc2 then throwError $ NoMonths pos
      ...
      d1@(VDOM rc1 _), (VDOM rc2 _) | rc1 == rc2 ->
        if allowed rc1 && allowed rc2 then pure $ BQD currentDomain (QF.BinaryCombinator functionName) left' right' (VDOM rc1 Nothing) (isFunctionalFunction functionName) True
...
compileComputationStep currentDomain (ComputationStep { functionName, arguments, computedType, start, end }) =
  if lookupHiddenFunctionIsEffect functionName then throwError (NotAFunction start end functionName)
  else do
    case (typeUri2ModelUri functionName) of
      Nothing -> throwError (NotWellFormedName start functionName)
      Just modelName ->
        if isExternalCoreModule modelName then do
          ...
          ( let
            ...
            in
              case mexpectedNrOfArgs of
                ...
                Just expectedNrOfArgs ->
                  if expectedNrOfArgs == length arguments || expectedNrOfArgs == length arguments - 1 then do
