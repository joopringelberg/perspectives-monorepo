...
newtype InvertedQuery = InvertedQuery
  ...
  -- When Just, this InvertedQuery was generated by invertCalculatedUsers for detecting new
  -- Calculated User role instances that arise when a role binding changes.
  -- The value is the Calculated User role type whose query was inverted.
  -- Nullable for backwards compatibility with stored InvertedQuery records that pre-date this field.
  , calculatedUserRoleType :: Maybe RoleType
  }
...
instance prettyPrintInvertedQuery :: PrettyPrint InvertedQuery where
  prettyPrint' t (InvertedQuery { description, users, states, statesPerProperty, perspectiveStartPosition, calculatedUserRoleType }) =
...
isStateQuery (InvertedQuery { users, calculatedUserRoleType }) = null users && isNothing calculatedUserRoleType

-- | True iff this InvertedQuery was generated by invertCalculatedUsers to detect new Calculated User
-- | role instances that arise when a role binding changes. Such queries have users=[] and a Just calculatedUserRoleType.
isCalculatedUserQuery :: InvertedQuery -> Boolean
isCalculatedUserQuery (InvertedQuery { calculatedUserRoleType }) = isJust calculatedUserRoleType
...
shouldResultInContextStateQuery (InvertedQuery { description, users, calculatedUserRoleType }) = null users && isNothing calculatedUserRoleType &&
  (isContextDomain $ range $ fromJust $ backwards description)

-- | This is a Partial function. Do not apply when the description has Nothing for its
-- | backwards part.
shouldResultInRoleStateQuery :: Partial => InvertedQuery -> Boolean
shouldResultInRoleStateQuery (InvertedQuery { description, users, calculatedUserRoleType }) = null users && isNothing calculatedUserRoleType &&
...
-- | True iff the forwards part of the QueryWithAKink is a FilterF operation.
-- | This is the case for InvertedQueries generated by `invertCalculatedUsers` when
-- | the Calculated User Role's query contains a filter: `storeInvertedQuery'` places
-- | the filter directly in the forward slot so that at runtime the role instance can
-- | be tested against the filter criterium before being recognised as a new user.
forwardStartsWithFilter :: InvertedQuery -> Boolean
forwardStartsWithFilter (InvertedQuery { description }) = case description of
  ZQ _ (Just (UQD _ FilterF _ _ _ _)) -> true
  _ -> false
