...
compileFunction (SQD _ (RolGetter (ENR (EnumeratedRoleType r))) _ _ _) =
...
compileFunction (BQD _ (BinaryCombinator ComposeF) f1 f2 _ _ _) =
...
compileFunction (BQD _ (BinaryCombinator SequenceF) f1 f2 _ _ _) = do
  ...
  then
    if (typeTimeOnly f2) then pure \c -> ArrayT $ pure []
    else compileFunction f2
  else do
    if (typeTimeOnly f2) then compileFunction f1
    else do
      f1' <- compileFunction f1
      f2' <- compileFunction f2
...
compileFunction (BQD _ (BinaryCombinator g) f1 f2 ran _ _) | isJust $ elemIndex g [ AddF, SubtractF, DivideF, MultiplyF ] = do
  ...
  if range f1 `eq` range f2 then pure $ performNumericOperation g ran f1' f2' (unsafePartial $ mapNumericOperator g ran)
...
performNumericOperation'
  ...
  -> Array String
  -> Array String
  -> (WriterT (ArrayWithoutDoubles InformedAssumption) MonadPerspectives) (Array String)
...
getDynamicPropertyGetter_ pt adt = do
  case pt of
    CP _ -> do
      ...
      if (isJust $ elemIndex pt allProps) || pt == (CP $ CalculatedPropertyType roleWithId) then getterFromPropertyType pt
      else pure (binding >=> f)
...
getDynamicEnumeratedPropertyGetter eprop adt rid = do
  ...
  case OBJ.lookup (unwrap eprop) aliases of
    ...
    Nothing -> do
      ...
      if isJust $ elemIndex (ENP eprop) allProps then do
