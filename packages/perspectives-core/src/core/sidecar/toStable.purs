...
instance ToStable ContextType where
  toStable ct@(ContextType stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableContextType }) -> case EM.lookup ct toStableContextType of
...
instance ToStable EnumeratedRoleType where
  toStable er@(EnumeratedRoleType stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableEnumeratedRoleType }) -> case EM.lookup er toStableEnumeratedRoleType of
...
instance ToStable CalculatedRoleType where
  toStable er@(CalculatedRoleType stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableCalculatedRoleType }) -> case EM.lookup er toStableCalculatedRoleType of
    Just stableEr -> pure stableEr
    Nothing -> do
...
instance ToStable EnumeratedPropertyType where
  toStable er@(EnumeratedPropertyType stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableEnumeratedPropertyType }) -> case EM.lookup er toStableEnumeratedPropertyType of
    Just stableEr -> pure stableEr
    Nothing -> do
...
instance ToStable CalculatedPropertyType where
  toStable er@(CalculatedPropertyType stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableCalculatedPropertyType }) -> case EM.lookup er toStableCalculatedPropertyType of
    Just stableEr -> pure stableEr
    Nothing -> do
...
instance ToStable StateIdentifier where
  toStable st@(StateIdentifier stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableStateIdentifier }) -> case EM.lookup st toStableStateIdentifier of
...
instance ToStable ViewType where
  toStable vt@(ViewType stableId) = lookupStableModelUri (ModelUri $ unsafePartial typeUri2ModelUri_ stableId) >>= getDomeinFile >>= \(DomeinFile { toStableViewType }) -> case EM.lookup vt toStableViewType of
