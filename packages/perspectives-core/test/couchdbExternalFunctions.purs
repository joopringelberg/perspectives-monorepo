...
uploadToRepository dfId@(ModelUri domeinFileName) = do
  if isModelUri domeinFileName then do
    ...
    case mdf of
      ...
      Right df -> void $ runMonadPerspectivesTransaction $ uploadToRepository_ (unsafePartial modelUri2ModelUrl domeinFileName) df [] emptyStableIdMapping Nothing
...
theSuite = suite "Perspectives.Extern.Couchdb" do
  ...
  test "upload model://perspectives.domains#System to repository from files" $ runP do
    ...
    dCache <- domeinCache
    if null errs then uploadToRepository (ModelUri "model://perspectives.domains#System")
  ...
  test "upload model://perspectives.domains#Parsing to repository from files" $ runP do
    ...
    if null errs then uploadToRepository (ModelUri "model://perspectives.domains#Parsing")
  ...
  test "upload model://perspectives.domains#TestPublicRole to repository from files" $ runP do
    ...
    if null errs then uploadToRepository (ModelUri "model://perspectives.domains#TestPublicRole")
  ...
  test "upload model://perspectives.domains#TestFields to repository from files" $ runP do
    ...
    if null errs then uploadToRepository (ModelUri "model://perspectives.domains#TestFields")
  ...
  test "upload model:perspectives.domains#BodiesWithAccounts to repository from files (without testuser)" $ runP do
    ...
    if null errs then uploadToRepository (ModelUri "model://perspectives.domains#BodiesWithAccounts")
  ...
  testOnly "upload model://perspectives.domains#CouchdbManagement_new to repository from files (without testuser)" $ runP do
    ...
    if null errs then uploadToRepository (ModelUri "model://perspectives.domains#CouchdbManagement")
