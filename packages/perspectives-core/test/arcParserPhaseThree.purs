...
import Test.Unit (Test, TestF, TestSuite, suite, test, testOnly)
...
theSuite = suite "Perspectives.Parsing.Arc.PhaseThree" do
          ...
          case r of
            ...
            (Right (DomeinFile dr')) -> do
              ...
              (runP $ phaseThree dr' state.postponedStateQualifiedParts Nil) >>=
                -- logShow x'
                case _ of
                  (Left [ e@(NotUniquelyIdentifyingRoleType _ _ _) ]) -> do
                    -- logShow e
                    assert "" true
                  otherwise -> do
                    assert "The binding of 'Binder' is not defined and that should have been detected." false
  ...
  --                     (Just pr) -> do
  --                       candidates <- runP $ withDomeinFile "model:MyTestDomain" (DomeinFile correctedDFR) ((lookForUnqualifiedPropertyType_ "Datum") (ENR $ EnumeratedRoleType "model:MyTestDomain$Feest"))
    ...
    \correctedDFR -> do
      ensureState "model:Test$Gast$SomeState" correctedDFR
        ...
        >>=
          case _ of
            (UQD _ qf bndg _ _ _) -> do
              assert "The queryfunction should be DeleteRole"
                ( case qf of
                    (DeleteRole _) -> true
                    otherwise -> false
                )
    ...
    \correctedDFR -> do
      ensureState "model:Test$Gast$SomeState" correctedDFR
        ...
        >>=
          case _ of
            (UQD _ qf bndg _ _ _) -> do
              assert "The queryfunction should be DeleteProperty"
                ( case qf of
        ...
        >>=
          case _ of
            (BQD _ qf val role _ _ _) -> do
              assert "The queryfunction should be AddPropertyValue"
                ( case qf of
