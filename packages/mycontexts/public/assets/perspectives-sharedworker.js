...
self.onconnect = function (e) {
  var connectionToAPage = e.ports[0];
  ...
  connectionToAPage.postMessage({
    responseType: "WorkerResponse",
    serviceWorkerMessage: "channelId",
    channelId: 1000000 * pageIndex
  });
  // start listening to the new page, handle requests.
  connectionToAPage.onmessage = function (requestFromPage) {
    return pdr.handleClientRequest(pdr, subscribingPages, requestFromPage, 1000000 * pageIndex);
  };
  // increment the index so we're ready for the next page that connects.
  pageIndex = pageIndex + 1;
};
//# sourceMappingURL=perspectives-sharedworker.js.map
